...
// ... (весь ваш существующий код до функции loop)

// Новые переменные для управления состоянием
let isPaused = false;
...
function resetGame() {
    if ((ball.x < 0 || ball.x > map.width) && !ball.isResetted) {
        // ИСПРАВЛЕНИЕ ЛОГИКИ: Очки должны прибавляться сопернику, а не сбрасывать счет в 0
        if (ball.x < 0) {
            rightCounter.textContent = parseInt(rightCounter.textContent) + 1;
        } else if (ball.x > map.width) {
            leftCounter.textContent = parseInt(leftCounter.textContent) + 1;
        ...
        setTimeout(() => {
            ball.x = map.width / 2;
            ball.y = map.height / 2;
            // Случайное направление при возрождении мяча
